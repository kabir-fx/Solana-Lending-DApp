...
use crate::{constants::{MAX_AGE, SOL_USD_FEED_ID, USDC_USD_FEED_ID}, errors::ErrorCode, state::{Bank, User}};
...
pub fn process_liquidate(ctx: Context<Liquidate>) -> Result<()> {
    ...
    let sol_price = price_update_account.get_price_no_older_than(&Clock::get()?, MAX_AGE, &sol_feed_id)?; 

    let usdc_feed_id = get_feed_id_from_hex(USDC_USD_FEED_ID)?;
    let usdc_price = price_update_account.get_price_no_older_than(&Clock::get()?, MAX_AGE, &usdc_feed_id)?;
    ...
    match ctx.accounts.collateral_mint.to_account_info().key() {
        key if key == liquidator_user_account.usdc_address => {
            let new_usdc_price = calculate_accrued_interest(liquidator_user_account.deposited_usdc, collateral_bank.interest_rate, liquidator_user_account.last_updated)?;

            total_collateral_value = new_usdc_price * usdc_price.price as u64;

            let new_sol_price = calculate_accrued_interest(liquidator_user_account.borrowed_sol, borrowed_bank.interest_rate, liquidator_user_account.last_updated)?;

            total_borrowed_value = new_sol_price * sol_price.price as u64;
        }
        _ => {
            let new_sol_price = calculate_accrued_interest(liquidator_user_account.deposited_sol, collateral_bank.interest_rate, liquidator_user_account.last_updated)?;

            total_collateral_value = new_sol_price * sol_price.price as u64;

            let new_usdc_price = calculate_accrued_interest(liquidator_user_account.borrowed_usdc, borrowed_bank.interest_rate, liquidator_user_account.last_updated)?;
    ...
    let health_factor = ((total_collateral_value as f64 * collateral_bank.liquidation_threshold as f64) / total_borrowed_value as f64) as f64;
    ...
    let liquidation_amount = total_borrowed_value.checked_mul(borrowed_bank.liquidation_close_factor).unwrap();
    ...
    let cpi_ctx_to_liquidator = CpiContext::new(ctx.accounts.token_program.to_account_info(), transfer_to_liquidator)
